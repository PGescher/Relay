...
const LoginPage: React.FC = () => {
  ...
  const onSubmit = async (data: LoginInput) => {
    ...
    try {
      ...
      // Nice UX: put cursor where it likely helps most
      if (res.status === 401) setFocus('password');
    } catch {
  ...
  return (
    <div className="min-h-screen bg-[var(--hero-bg)] text-[var(--hero-text)] flex flex-col items-center justify-center px-8 relative overflow-hidden font-inter">
      ...
      <div className="absolute bottom-[22%] right-[18%] w-2 h-2 bg-rose-400 rounded-full opacity-15 animate-float" style={{ animationDelay: '2.6s' }} />
      <div className="absolute top-[62%] right-[12%] w-1 h-1 bg-white rounded-full opacity-10 animate-float" style={{ animationDelay: '1.1s' }} />
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="p-8 space-y-6">
            ...
            <div>
              ...
              <div className="mt-3 flex items-center justify-between">
                <div className="text-[10px] font-[900] uppercase tracking-[0.45em] text-white/25">
                  
                </div>
                {/* Optional: wire later */}
                <button
                  type="button"
                  className="text-[10px] font-[900] uppercase tracking-[0.45em] text-white/40 hover:text-white transition-colors"
